...
parser.add_argument("method",choices=['wynerdca','gd','wynervi'])
parser.add_argument("--maxiter",type=int,default=10000,help="maximum iteration before termination")
...
parser.add_argument("--output_dir",type=str,default="wynerCI_threshold_results",help="output filename")
parser.add_argument("--seed",type=int,default=None,help="random seed for reproduction")
parser.add_argument("--ny",type=int,default=3,help="number of uniform hidden labels")
...
parser.add_argument("--betamin",type=float,default=0.01,help="minimum trade-off parameter for search")
parser.add_argument("--betamax",type=float,default=10.0,help="maximum trade-off parameter for search")
parser.add_argument("--numbeta",type=int,default=50,help="number of search points")
...
# we need the joint mutual information, for all bipartite
...
nz_set = np.arange(2,max(args.ny,np.prod(prob_joint.shape))+2,1)
...
tuple_list = set_dict['tuple_list']
d_p_pair = set_dict['cond_list']
d_mixs = set_dict['mi'] # should be a list
d_ents = [] # used to compute H(X|Z)
d_ent_xv = ut.calcEnt(prob_joint)
for idx, (ws,wc) in enumerate(tuple_list):
	#(pws,pwc) = d_p_pair[idx]
	pws = np.sum(prob_joint,axis=wc)
	pwc = np.sum(prob_joint,axis=ws)
	# get separate marginal
	d_ents.append([ut.calcEnt(pws),ut.calcEnt(pwc)])
	ws_tex = "".join(["{:}".format(item) for item in ws])
	wc_tex = "".join(['{:}'.format(item) for item in wc])
	res_dict["H_W{:}".format(ws_tex)] = []
	res_dict['H_W{:}'.format(wc_tex)] = []
	res_dict["H_W{:}|Z".format(ws_tex)] = []
	res_dict["H_W{:}|Z".format(wc_tex)] = []
	res_dict['IZW{:}'.format(ws_tex)] = []
	res_dict["IZW{:}".format(wc_tex)] = []
	res_dict['IW{:}_W{:}|Z'.format(ws_tex,wc_tex)] = []
	res_dict['IW{:}_W{:}'.format(ws_tex,wc_tex)] = []
...
for beta in gamma_range:
	for nz in nz_set:
		for nn in range(args.nrun):
			...
			status_tex = ["nz,{:},beta,{:.3f},conv,{:},nit,{:},DKL,{:.3e},IW,{:.3e}".format(
							nz,beta,out_dict['conv'],out_dict['niter'],dkl_error,wy_mi)]
			res_dict['nz'].append(nz)
			res_dict['beta'].append(beta)
			res_dict['conv'].append(out_dict['conv'])
			res_dict['niter'].append(out_dict['niter'])
			res_dict['rt'].append(rt_dt)
			res_dict["HZ"].append(entz)
			res_dict["HZ|XV"].append(entzcxv)
			res_dict['HXV'].append(d_ent_xv)
			res_dict["IZXV"].append(entz - entzcxv)
			condmi_list =[]
			for cidx, item in enumerate(comp_mizw):
				...
				# conditional
				ws,wc = tuple_list[cidx]
				ws_tex = "".join(["{:}".format(item) for item in ws])
				wc_tex = "".join(['{:}'.format(item) for item in wc])
				(ent_ws,ent_wc) = d_ents[cidx]
				res_dict["H_W{:}".format(ws_tex)].append(ent_ws)
				res_dict['H_W{:}'.format(wc_tex)].append(ent_wc)
				res_dict["H_W{:}|Z".format(ws_tex)].append(ent_ws-mizw)
				res_dict["H_W{:}|Z".format(wc_tex)].append(ent_wc-mizw_cmpl)
				res_dict['IZW{:}'.format(ws_tex)].append(mizw)
				res_dict["IZW{:}".format(wc_tex)].append(mizw_cmpl)
				res_dict['IW{:}_W{:}|Z'.format(ws_tex,wc_tex)].append(cmi)
				res_dict['IW{:}_W{:}'.format(ws_tex,wc_tex)].append(mixx)
...
safe_savename = ut.getSafeSaveName(args.output_dir,savename_base,'.pkl')
...
tmp_df = pd.DataFrame.from_dict(res_dict)
tmp_df.to_csv(os.path.join(d_save_dir,safe_savename+".csv"),index=False)
